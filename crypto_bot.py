...
import os
from flask import Flask # 👈 این کتابخونه جدید برای سایت الکیه
...
# --- 🌐 وب‌سرور ضد خواب (Fake Web Server) ---
app = Flask(__name__)
...
def calculate_rsi(symbol, period=14):
    try:
        url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval=1h&limit={period+1}"
...
def handle_crypto_request(message):
    ...
    if text in COIN_BUTTONS:
        bot.reply_to(message, t['wait'])
        try:
            coin_symbol = text.split()[1] + 'USDT'
            url = f"https://api.binance.com/api/v3/ticker/24hr?symbol={coin_symbol}"
            data = requests.get(url).json()
...
if __name__ == '__main__':
    keep_alive() # روشن کردن سرور وب الکی!
    print("="*50)
    print(" ☁️ CLOUD BOT IS ONLINE 24/7 ")
    print("="*50)
